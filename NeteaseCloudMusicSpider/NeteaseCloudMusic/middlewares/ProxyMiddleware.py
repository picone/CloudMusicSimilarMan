...
class ProxyMiddleware(object):
    def __init__(self, redis_host, redis_port, redis_password, redis_db):
        ...
        self.__fetcher = ProxyFetcher('http', redis_args=args)
        self.__start_time = 0
        self.__proxy = self._get_next_proxy()
    ...
    def process_request(self, request, spider):
        ...
        if 'retry_times' in request.meta and request.meta['retry_times'] > 3:
            ...
            self.__proxy = self._get_next_proxy()
        request.meta['proxy'] = self.__proxy
        return None
    ...
    def process_response(self, request, response, spider):
        if isinstance(response, Response):
            ...
            if response.status == 200:
                if b'"code":460' in response.body:
                    self.__fetcher.proxy_feedback('failure', request.meta['proxy'], self._get_cost_time())
                    self.__proxy = self._get_next_proxy()
                    request.meta['proxy'] = self.__proxy
                    spider.logger.info('proxy %s is banned', request.meta['proxy'])
                    return request
                else:
